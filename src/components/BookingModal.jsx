...
const BookingModal = ({ court, onClose, refetch }) => {
  ...
  const mutation = useMutation({
    mutationFn: async (bookingData) => {
      ...
      console.log('Frontend Firebase JWT Token:', token)

      // const res = await axios.post('https://my-assignment-12-server-kappa.vercel.app/bookings', bookingData, {
      const res = await axios.post('https://my-assignment-12-server-kappa.vercel.app/bookings', bookingData, {
    ...
    onError: (error) => {
      Swal.fire('Error!', error.response?.data?.error || error.message || 'Something went wrong.', 'error')
    }
  })
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded shadow-md w-80 md:w-96 relative">
        <button className="absolute top-2 right-2 text-xl" onClick={onClose}>✖</button>
        <h2 className="text-lg font-semibold mb-4">Book {court.name}</h2>

        <form onSubmit={handleSubmit} className="space-y-3 text-sm">
          <div>
            <label className="block font-medium">Court Type:</label>
          ...
          <div>
            <label className="block font-medium">Slots:</label>
            <input
              type="number"
              min={1}
              className="border px-2 py-1 w-full"
          ...
          >
            {mutation.isLoading ? 'Sending...' : 'Send Booking Request'}
          </button>
        </form>
      </div>
    </div>

...
//     (id) => axiosSecure.patch(`/bookings/approve/${id}`),
//     {
//       onSuccess: () => {
...
//     (id) => axiosSecure.delete(`/bookings/${id}`),
//     {
//       onSuccess: () => {
...
//         </thead>
//         <tbody>
//           {bookings.map(({ _id, userName, courtName, slots, date, price }) => (
//             <tr key={_id} className="hover:bg-gray-50">
//               <td className="border px-3 py-2">{userName}</td>
...
//   // GET bookings with object argument
//   const { data: bookings = [], isLoading, isError } = useQuery({
//     queryKey: ['pendingBookings'],
//     queryFn: fetchPendingBookings,
//   });

//   // APPROVE mutation with object argument
//   const approveMutation = useMutation({
...
//   // REJECT mutation with object argument
//   const rejectMutation = useMutation({
...
//   if (isLoading) return <p className="p-4">Loading bookings...</p>;
//   if (isError) return <p className="p-4 text-red-600">Failed to load bookings.</p>;
//   if (bookings.length === 0) return <p className="p-4">No pending bookings.</p>;
...
//             <tr key={_id} className="hover:bg-gray-50">
//               <td className="border px-3 py-2">{userName || user?.name || 'Unknown User'}</td>
//               <td className="border px-3 py-2">{courtName}</td>
//               <td className="border px-3 py-2">{slots}</td>
//               <td className="border px-3 py-2">{new Date(date).toLocaleDateString()}</td>
//               <td className="border px-3 py-2">৳{price}</td>
...
// ইমেইল থেকে ইউজারনেম বের করার ফাংশন
